...
AccessControlProcedure.prototype.authorize = function(obj, action, roles, callback) {
  console.log('Running authorization procedure', obj, action, roles)

  this._nextACL(obj, action, roles, this._accessControls.slice(0), undefined, function(err, details) {
    callback(err, details)
  })
}

AccessControlProcedure.prototype._nextACL = function(obj, action, roles, accessControls, details, callback) {
  if(!details) {
  ...
  if(accessControls && accessControls.length > 0) {
    ...
    if(accessControl.shouldApply(obj, action)) {
      console.log('running authorization service', accessControl.name())
      accessControl.authorize(obj, action, roles, function(err, result) {
        ...
        if(stop) {
          ...
          self._nextACL(obj, action, roles, accessControls, details, callback)
        }
      })
    } else {
      console.log('ignoring authorization service', accessControl.name())
      self._nextACL(obj, action, roles, accessControls, details, callback)
    }
  } else {
    callback(undefined, details)
