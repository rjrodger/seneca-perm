...
ACLMicroservicesBuilder.prototype._executeReadPermissions = function(args, callback) {
  ...
  this._seneca.act(args, function(err, entity) {
    ...
    if(err || !entity) {
      ...
      self._deepAuthorize(entityDef, entity, args.cmd, args.cmd, roles, context, function(err, entity) {
        callback(err, entity)
      })
  ...
  this._seneca.act(args, function(err, entities) {
    ...
    if(err) {
      ...
      for(var i = 0 ; i < entities.length ; i++) {
        self._deepAuthorize(entityDef, entities[i], args.cmd, args.cmd, roles, context, processAuthResultForEntity)
      }
...
ACLMicroservicesBuilder.prototype._executeSavePermissions = function(args, callback) {
  ...
  if(args.ent.id) { // update
    ...
    this._loadAndAuthorize(entityDef, args.ent.id, args.cmd, roles, context, function(err, entity) {
      ...
      if(err) {
        ...
        self._deepAuthorize(entityDef, args.ent, args.cmd, 'save_new', roles, context, function(err, entity) {
...
ACLMicroservicesBuilder.prototype._loadAndAuthorize = function(entityDef, entityId, action, roles, context, callback) {
  ...
  ent.load$(entityId, function(err, entity) {
    ...
    self._deepAuthorize(entityDef, entity, action, ruleAction, roles, context, function(err, entity) {
      callback(err, entity)
    })
  })
}

ACLMicroservicesBuilder.prototype._deepAuthorize = function(entityDef, entity, action, ruleAction, roles, context, callback) {
  var self = this

  var aclProcedure = AccessControlProcedure.getProcedureForEntity(self._ACLProcedureResolver, entityDef, action)

  debug('_deepAuthorize', action, roles, aclProcedure ? 'with procedure' : 'no procedure')
  if(aclProcedure) {
    aclProcedure.authorize(entity, ruleAction, roles, context, function(err, authDecision) {
      if(err) {
        ...
        aclProcedure.applyFilters(authDecision.filters, entity)
