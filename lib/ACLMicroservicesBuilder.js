...
ACLMicroservicesBuilder.prototype._deepAuthorize = function(entityDef, entity, action, ruleAction, roles, context, applyFilters, callback) {
  ...
  for(var i = 0; i < allowedProperties.length; i++) {
    if(allowedProperties[i]['entities'][0]['zone'] === entityDef['zone'] && allowedProperties[i]['entities'][0]['base'] === entityDef['base'] && allowedProperties[i]['entities'][0]['name'] === entityDef['name']) {
      allowedFields = allowedProperties[i].fields; 
    }
  }
